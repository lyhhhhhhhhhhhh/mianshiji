...
import com.yupi.mianshiji.mapper.QuestionBankQuestionMapper;
...
//        }
    ...
    public void batchAddQuestionToBank(List<Long> questionIdList, long questionBankId, User loginUser) {
        ...
        ThrowUtils.throwIf(loginUser == null, ErrorCode.NOT_LOGIN_ERROR, "用户不能为空");
        //检查题目id是否存在
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void batchDeleteQuestionFromBank(List<Long> questionIdList, long questionBankId) {
        //参数校验
        ThrowUtils.throwIf(CollUtil.isEmpty(questionIdList), ErrorCode.PARAMS_ERROR, "题目列表不能为空");
        ThrowUtils.throwIf(questionBankId <= 0, ErrorCode.PARAMS_ERROR, "题库id不能为空");
        //执行删除关联数据
